package com.ewancli.agent;

import com.ewancli.llm.LlmClient;
import com.ewancli.llm.LlmTraceLogger;
import com.ewancli.context.ContextProfile;
import com.ewancli.context.TokenUsageFormatter;
import com.ewancli.history.ConversationLedger;
import com.ewancli.lsp.LspDiagnosticReport;
import com.ewancli.memory.AutoCompactionManager;
import com.ewancli.memory.ExplicitMemoryHints;
import com.ewancli.memory.MemoryManager;
import com.ewancli.prompt.PromptAssembler;
import com.ewancli.prompt.PromptContext;
import com.ewancli.prompt.PromptMode;
import com.ewancli.prompt.ProjectMemoryLoader;
import com.ewancli.render.PlainRenderer;
import com.ewancli.render.Renderer;
import com.ewancli.render.StatusInfo;
import com.ewancli.runtime.CancellationContext;
import com.ewancli.skill.SkillContextBuffer;
import com.ewancli.skill.SkillIndexFormatter;
import com.ewancli.skill.SkillRegistry;
import com.ewancli.util.AnsiStyle;
import com.ewancli.tool.ToolRegistry;
import com.ewancli.tool.ToolRegistry.ToolExecutionResult;
import com.ewancli.tool.ToolRegistry.ToolInvocation;
import com.ewancli.tool.TurnToolPolicy;
import com.ewancli.util.TerminalMarkdownRenderer;
import com.ewancli.image.ImageReferenceParser;
...
public class Agent {
    ...
    private String buildProjectMemoryContext() {
        try {
            ...
            log.warn("Failed to load EWAN.md project memory", e);
            return "";
        }
    }
    ...
    public String getContextStatus() {
        com.ewancli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.ewancli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.ewancli.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.ewancli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.ewancli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.ewancli.memory.MemoryEntry.estimateTokens(
