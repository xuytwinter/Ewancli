 ...
 * 1. 系统属性：{@code ewancli.react.token.budget} / {@code ewancli.react.stagnation.window} /
 *    {@code ewancli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dewancli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dewancli.react.token.budget=N / -Dewancli.react.hard.max.iterations=N
        // 仍可为无人值守或成本敏感场景启用硬预算。
        return new AgentBudget(
                readIntProperty("ewancli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("ewancli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("ewancli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
