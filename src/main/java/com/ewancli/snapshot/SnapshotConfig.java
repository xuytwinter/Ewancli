...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("ewancli.snapshot.enabled", "EWANCLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("ewancli.snapshot.dir", "EWANCLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".ewancli", "snapshots").toString()));
        int max = readInt("ewancli.snapshot.max", "EWANCLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("ewancli.snapshot.excludes", "EWANCLI_SNAPSHOT_EXCLUDES", ""));
