...
class McpClientTest {
    ...
    void initializeTimeoutCanBeOverriddenBySystemProperty() {
        String previous = System.getProperty("ewancli.mcp.initialize.timeout.seconds");
        try {
            System.setProperty("ewancli.mcp.initialize.timeout.seconds", "17");
            assertEquals(17, McpClient.initializeTimeoutSeconds());
        } finally {
            if (previous == null) {
                System.clearProperty("ewancli.mcp.initialize.timeout.seconds");
            } else {
                System.setProperty("ewancli.mcp.initialize.timeout.seconds", previous);
            }
        }
    }
    ...
    void closeIsBestEffortAndDoesNotBlockOnServerSilence() throws Exception {
        // close 不再发 shutdown notification（server 卡死时会让 Ewancli 退出阻塞）。
