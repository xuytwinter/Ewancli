...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("ewancli.audit.dir");
        System.setProperty("ewancli.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("ewancli.audit.dir");
            } else {
                System.setProperty("ewancli.audit.dir", previous);
            }
        }
    }
